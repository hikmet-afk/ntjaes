...
client.on("ready", async () => {
  ...
  client.user.setActivity(`Why 💖`, { type: "PLAYING" });
...
client.on("ready", () => {
        client.channels.cache.get("818409654287597599").join();
      })

// tag rol

client.on("userUpdate", async (stg, yeni) => {
  var sunucu = client.guilds.cache.get('815971195647819816'); // Buraya Sunucu ID
  var uye = sunucu.members.cache.get(yeni.id);
  var ekipTag = "て"; // Buraya Ekip Tag
  var ekipRolü = "815971195685830658"; // Buraya Ekip Rolünün ID
  var logKanali = "815971199313510406"; // Loglanacağı Kanalın ID
  ...
  if ((yeni.username).includes(ekipTag) && !uye.roles.cache.has(ekipRolü)) {
    try {
      ...
      await uye.send(`Tagımızı aldığın için teşekkürler! Aramıza hoş geldin. İyi Vakitler Geçirmen Dileğiyle.`);
